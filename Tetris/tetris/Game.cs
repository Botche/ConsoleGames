...
namespace Tetris
{
    class Game
    {
        ...
        static bool[,] GameField = new bool[GameConstraits.TetrisRows, GameConstraits.TetrisCols];

        static void Main(string[] args)
        {
            var tetrisObjects = new TetrisObjects();
            ...
            ConsoleWindow.SetGameSettings();
            CurrentFigure = tetrisObjects.GetCurrentFigure();
            ...
            while (true)
            {
                ...
                if (Console.KeyAvailable)
                {
                    ...
                    {
                        if (CurrentFigureCol < GameConstraits.TetrisCols - CurrentFigure.GetLength(1))
                        {
                            CurrentFigureCol++;
                        }
                    }
            ...
            for (int row = 0; row < GameField.GetLength(0); row++)
            {
                ...
                if (rowIsFull)
                {
                    for (int rowToMove = row; rowToMove >= 1; rowToMove--)
                    {
                        for (int col = 0; col < GameField.GetLength(1); col++)
                        {
                            GameField[rowToMove, col] = GameField[rowToMove - 1, col];
                        }
                    }
        ...
        static void AddCurrentFigureToTetrisField()
        {
            for (int row = 0; row < CurrentFigure.GetLength(0); row++)
            {
                for (int col = 0; col < CurrentFigure.GetLength(1); col++)
                {
                    ...
                    {
                        GameField[CurrentFigureRow + row, CurrentFigureCol + col] = true;
                    }
                }
            }
        }
        ...
        {
            if (CurrentFigureCol > GameConstraits.TetrisCols - figure.GetLength(1))
            {
                return true;
            }

            if (CurrentFigureRow + figure.GetLength(0) == GameConstraits.TetrisRows)
            {
                return true;
            }
            ...
            for (int row = 0; row < figure.GetLength(0); row++)
            {
                ...
                {
                    if (figure[row, col] &&
                        GameField[CurrentFigureRow + row + 1, CurrentFigureCol + col])
                    {
                        return true;
                    }
        ...
        static void DrawBorder()
        {
            ...
            string line = "╔";
            line += new string('═', GameConstraits.TetrisCols);

            line += "╦";
            line += new string('═', GameConstraits.InfoCols);
            line += "╗";
            Console.Write(line);

            for (int i = 0; i < GameConstraits.TetrisRows; i++)
            {
                string middleLine = "║";
                middleLine += new string(' ', GameConstraits.TetrisCols);
                middleLine += "║";
                middleLine += new string(' ', GameConstraits.InfoCols);
                middleLine += "║";
                Console.Write(middleLine);
            }

            string endLine = "╚";
            endLine += new string('═', GameConstraits.TetrisCols);
            endLine += "╩";
            endLine += new string('═', GameConstraits.InfoCols);
            endLine += "╝";
        ...
        static void DrawInfo()
        {
            ...
            Write("Level:", 1, 3 + GameConstraits.TetrisCols);
            Write(Level.ToString(), 2, 3 + GameConstraits.TetrisCols);
            Write("Score:", 4, 3 + GameConstraits.TetrisCols);
            Write(Score.ToString(), 5, 3 + GameConstraits.TetrisCols);
            Write("Best:", 7, 3 + GameConstraits.TetrisCols);
            Write(HighScore.ToString(), 8, 3 + GameConstraits.TetrisCols);
            Write("Frame:", 10, 3 + GameConstraits.TetrisCols);
            Write(Frame.ToString() + " / " + (FramesToMoveFigure - Level).ToString(), 11, 3 + GameConstraits.TetrisCols);
            Write("Position:", 13, 3 + GameConstraits.TetrisCols);
            Write($"{CurrentFigureRow}, {CurrentFigureCol}", 14, 3 + GameConstraits.TetrisCols);
            Write("Keys:", 16, 3 + GameConstraits.TetrisCols);
            Write($"  ^ ", 18, 3 + GameConstraits.TetrisCols);
            Write($"<   > ", 19, 3 + GameConstraits.TetrisCols);
            Write($"  v  ", 20, 3 + GameConstraits.TetrisCols);
        }
